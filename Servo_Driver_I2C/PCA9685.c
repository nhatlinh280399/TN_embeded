...
static struct cdev etx_cdev_PCA9685;
...
static int __init etx_driver_init(void)
{
        ...
        if((alloc_chrdev_region(&dev, 0, 1, "PCA9685")) <0){
        ...
        cdev_init(&etx_cdev_PCA9685,&fops);

        /*Adding character device to the system*/
        if((cdev_add(&etx_cdev_PCA9685,dev,1)) < 0){
            printk(KERN_INFO "Cannot add the device to the system\n");
            goto r_class;
        }

        /*Creating struct class*/
        if((dev_class = class_create(THIS_MODULE,"PCA9685")) == NULL){
            printk(KERN_INFO "Cannot create the struct class\n");
            goto r_class;
        }

        /*Creating device*/
        if((device_create(dev_class,NULL,dev,NULL,"PCA9685")) == NULL){
...
MODULE_LICENSE("GPL");
MODULE_AUTHOR("Lab411 <ktttlab411@hust.edu.com>");
MODULE_DESCRIPTION("PCA9685 Device Driver");
MODULE_VERSION("1.3");